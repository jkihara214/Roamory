...
type VerificationStatus = "success" | "already_verified" | "invalid" | "error" | "loading";
...
export default function EmailVerificationPage() {
  ...
  useEffect(() => {
    ...
    if (statusParam && ["success", "already_verified", "invalid", "error"].includes(statusParam)) {
  ...
  const statusConfigs: Record<VerificationStatus, StatusConfig> = {
    loading: {
      icon: <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500"></div>,
      title: "認証中...",
    ...
    invalid: {
      ...
      title: "無効な認証リンク",
      message: "認証リンクが無効です。新しい認証メールをリクエストしてください。",
    ...
    <>
      ...
      <div className="flex flex-col items-center py-8 px-4 min-h-[calc(100vh-64px)]">
        <div className="w-full max-w-md mx-auto">
          <div className={`p-8 rounded-2xl shadow-xl border ${config.bgColor} ${config.borderColor}`}>
            {/* アイコンとタイトル */}
            ...
            {email && (
              <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
                <p className="text-sm text-gray-600 mb-1">認証されたメールアドレス:</p>
