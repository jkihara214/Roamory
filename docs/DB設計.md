...
### 🗺️ countries テーブル運用補足

**geojson_url カラムの実際の運用：**

- **現在の実装方針**：
  - **GeoJSON データはデータベースで管理しない**
  - フロントエンドは`/data/world-countries-110m.geojson`を直接読み込み
  - `geojson_url`カラムは将来的な個別国データ管理や CDN 移行時のために予約済み
- **データ管理戦略**：

  - **Natural Earth 110m 精度**：`world-countries-110m.geojson`（258 カ国・709KB）
  - **静的ファイル配信**：シンプル・高速・インフラコスト削減
  - **単一ファイル構成**：フォールバック処理不要でコードが簡潔

- **ISO_A2 コード連携**：
  - `countries.code`と GeoJSON の`properties.ISO_A2`で突合
  - Natural Earth データとの整合性を保証

**データ更新方針：**

- Natural Earth は年 1-2 回更新される公式データセット
- 更新時は以下コマンドで最新版を取得：
  ```bash
